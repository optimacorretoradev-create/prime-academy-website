...
export async function CoursesSection() {
  ...
  return (
    <section className="py-20 md:py-28 bg-background relative overflow-hidden">
      {/* Decorative gradient */}
      <div className="absolute top-0 right-1/4 w-[500px] h-[500px] bg-accent/5 rounded-full blur-3xl -translate-y-1/2" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {courses.slice(0, 6).map((course, index) => (
            <AnimatedCard key={course.id} delay={index * 0.1}>
              <Card className="h-full flex flex-col justify-between border border-border/80 hover:border-accent/40 hover:shadow-xl transition-all duration-300 group rounded-3xl bg-card overflow-hidden">
                <div>
                  ...
                  <div className="relative h-48 w-full overflow-hidden">
                    ...
                    <div className="absolute top-4 left-4 z-20">
                      <Badge className={`border uppercase tracking-wider text-[9px] font-extrabold px-3 py-1 shadow-sm rounded-full ${getCategoryBadgeStyle(course.category)}`}>
                        {course.category}
                      </Badge>
                    </div>
                  </div>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    {/* Highly descriptive visual metadata lines */}
                    <div className="grid grid-cols-3 gap-2 pt-2 border-t border-border/50 text-[11px] text-muted-foreground font-semibold">
                      <div className="flex items-center gap-1.5 bg-muted/40 p-2 rounded-xl border border-border/40">
                        <Clock className="h-3.5 w-3.5 text-accent flex-shrink-0" />
                        <span>{course.duration}</span>
                      </div>
                      <div className="flex items-center gap-1.5 bg-muted/40 p-2 rounded-xl border border-border/40 justify-center">
                ...
                <CardFooter className="flex flex-col gap-3 pt-2 pb-6 px-6 border-t border-border/50 mt-4 bg-muted/10">
                  ...
                  <Button asChild className="w-full bg-accent text-accent-foreground hover:bg-accent/90 rounded-2xl font-bold h-11 shadow-sm group-hover:shadow-md transition-all">
